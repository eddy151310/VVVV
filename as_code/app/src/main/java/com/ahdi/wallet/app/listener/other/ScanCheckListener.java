...
public class ScanCheckListener implements HttpReqTaskListener {

    private static final String TAG = ScanCheckListener.class.getSimpleName();

    private OtherSdkCallBack callBack;

    public ScanCheckListener(OtherSdkCallBack callBack) {
    ...
    public void onPostExecute(JSONObject json) {
        if (json != null) {
            LogUtil.e(TAG, "ScanCheckListener响应数据: " + json.toString());
        }
        if (callBack == null) {
            return;
        }
        ScanCheckRsp resp = ScanCheckRsp.decodeJson(ScanCheckRsp.class, json);
        if (resp != null) {
