...
public class PassPortAuthFragment extends BaseFragment implements View.OnClickListener{

    public static final String TAG = PassPortAuthFragment.class.getSimpleName();
    private static final String rnaType = "Passport";

    /**身份证姓名和身份证号码填写区域*/
    private EditText et_name,et_card_number;
    private ImageView iv_name_delete,iv_number_delete;
    /**id card 显示区域*/
    private RelativeLayout rl_passport_area;
    private LinearLayout ll_take_photo;
    private ImageView iv_card, iv_retake_card;

    /**手持身份证显示区域*/
    private RelativeLayout rl_hold_passport_area;
    private LinearLayout ll_take_hold_photo;
    private ImageView iv_hold_card, iv_retake_hold_card;

    private Button btn_confirm;
    private LoadingDialog loadingDialog = null;

    private Bitmap passPortCardBitmap = null;
    private Bitmap holdPhotoBitmap = null;

    @Override
    protected View initRootView(LayoutInflater inflater, ViewGroup container) {
        return inflater.inflate(R.layout.fragment_passport_auth, container, false);
    }
    ...
    protected void initView(View view) {
        ...
        //拍证件区域
        rl_passport_area = view.findViewById(R.id.rl_passport_area);
        rl_passport_area.setOnClickListener(this);
        ...
        //拍手持证件区域
        rl_hold_passport_area = view.findViewById(R.id.rl_hold_passport_area);
        rl_hold_passport_area.setOnClickListener(this);
        ll_take_hold_photo = view.findViewById(R.id.ll_take_hold_photo);
    ...
    public void onClick(View v) {
        ...
        if (id == R.id.btn_confirm) {
            submit();
        }else if (id == R.id.rl_passport_area || id == R.id.iv_retake_card) {
            TakePhotoMain.getInstance().takePhoto(mActivity, TakePhotoMain.TYPE_PASSPORT_CARD, photoCallBack);
            ...
            iv_number_delete.setVisibility(View.GONE);
        }else if (id == R.id.rl_hold_passport_area || id == R.id.iv_retake_hold_card) {
            TakePhotoMain.getInstance().takePhoto(mActivity, TakePhotoMain.TYPE_HOLD_PASSPORT_PHOTO, photoCallBack);
        }
    }
    ...
    private TakePhotoCallBack photoCallBack = new TakePhotoCallBack() {
        ...
        public void onResult(String Code, int type, String reason, Bitmap bitmap, String photoPath) {
            ...
            if (type == TakePhotoMain.TYPE_PASSPORT_CARD){
                onIDCardPhoto(bitmap);
            }else if (type == TakePhotoMain.TYPE_HOLD_PASSPORT_PHOTO){
    ...
    private void onIDCardPhoto(Bitmap bitmap){
        ...
        passPortCardBitmap = bitmap;
        rl_passport_area.setOnClickListener(null);
    ...
    private void onHoldIDPhoto(Bitmap bitmap) {
        ...
        rl_hold_passport_area.setOnClickListener(null);
        ll_take_hold_photo.setVisibility(View.GONE);
    ...
    private void submitBtnStatus() {
        ...
        if (!TextUtils.isEmpty(et_name.getText().toString()) &&
                !TextUtils.isEmpty(et_card_number.getText().toString()) &&
                card_number.length() >= 6 &&
                passPortCardBitmap != null && holdPhotoBitmap != null) {
    ...
    private void submit() {
        ...
        IDVerifySdkMain.getInstance().onRnaSubmitAll(rnaInfoSchema, ImageUtil.bitmapToString(passPortCardBitmap),ImageUtil.bitmapToString(holdPhotoBitmap) , rnaType, null,ProfileUserUtil.getInstance().getSID(),new IDVerifyCallBack() {
            @Override
    ...
    public void onDetach() {
        super.onDetach();
        if (passPortCardBitmap != null && !passPortCardBitmap.isRecycled()){
            passPortCardBitmap.recycle();
        }
        if (holdPhotoBitmap != null && !holdPhotoBitmap.isRecycled()){
            holdPhotoBitmap.recycle();
        }
        passPortCardBitmap = null;
