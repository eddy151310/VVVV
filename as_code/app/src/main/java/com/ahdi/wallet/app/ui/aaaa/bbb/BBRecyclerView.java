...
public class BBRecyclerView extends RecyclerView {
    ...
    public BBRecyclerView(Context context, AttributeSet attrs) {
        this(context, attrs, 0);
    }

    public BBRecyclerView(Context context, AttributeSet attrs, int defStyle) {
        ...
        setOnScrollListener(new OnScrollListener() {
            ...
            public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
                ...
                if (getAdapter() != null && getLayoutManager() != null) {
                     ...
                     * lastVisiblePosition + 2 >= itemCount - 1 最后可见+2 >= 总条数 加载更多
                     * distanceY < 0 为上拉的时候才刷新
                     */
                    if (distanceY < 0 && itemCount != 0 && lastVisiblePosition + 2 >= itemCount - 1 && !isLoadingMore && loadMoreEnable && hasMoreData) {
                        //正在加载更多
                        loading();
    ...
    public class AutoLoadAdapter extends Adapter<ViewHolder> {
         ...
         * 底部没有更多数据布局
         */
        private final int TYPE_NO_MORE_DATA_FOOTER = Integer.MAX_VALUE - 2;
        private final int TYPE_NO_DATA_FOOTER = Integer.MAX_VALUE - 3;
        ...
        public int getItemViewType(int position) {
            if (position == getItemCount() - 1 && loadMoreEnable && footerResource != -1 && footer_visible && hasMoreData) {
                return TYPE_FOOTER;
            }
            if (position == getItemCount() - 1 && !hasMoreData && footerResource != -1){
                return TYPE_NO_MORE_DATA_FOOTER;
            }else if(position == getItemCount() - 1 && footerResource == -1){
                return TYPE_NO_DATA_FOOTER;
            }
            if (dataAdapter.getItemViewType(position) == TYPE_FOOTER) {
                throw new RuntimeException("adapter中itemType不能为:" + TYPE_FOOTER);
            }
            if (dataAdapter.getItemViewType(position) == TYPE_NO_MORE_DATA_FOOTER) {
                throw new RuntimeException("adapter中itemType不能为:" + TYPE_NO_MORE_DATA_FOOTER );
            }
        ...
        public ViewHolder onCreateViewHolder(ViewGroup parent, int viewType) {
            ...
            } else if(viewType == TYPE_NO_MORE_DATA_FOOTER){//没有更多数据
                holder = new FooterViewHolder(LayoutInflater.from(getContext()).inflate(R.layout.layout_no_more_data_footer, parent, false));
            }else if(viewType == TYPE_NO_DATA_FOOTER){//没有数据
                holder = new FooterViewHolder(LayoutInflater.from(getContext()).inflate(R.layout.layout_empty_data,parent,false));
            }else{//数据
        ...
        public void onBindViewHolder(ViewHolder holder, int position) {
            ...
            if (itemViewType != TYPE_FOOTER && itemViewType != TYPE_NO_MORE_DATA_FOOTER) {
