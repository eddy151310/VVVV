...
public final class CameraConfigurationManager {
    ...
    private static double MAX_PREVIEW_PIXELS ;
    ...
    public void initFromCameraParameters(Camera camera) {
        ...
        MAX_PREVIEW_PIXELS =  screenResolution.x * screenResolution.y ;
    ...
    public void setDesiredCameraParameters(Camera camera, boolean safeMode) {
        ...
        if (parameters == null) {
            LogUtil.d(TAG, "Device error: 没有相机参数。没有进行配置。");
            return;
        }

        if (safeMode) {
            LogUtil.d(TAG, "In camera config safe mode -- most settings will not be honored");
        }
        ...
        if (prefs.getBoolean(QRConfig.KEY_INVERT_SCAN, false)) {
    ...
    private Point findBestPreviewSizeValue(Camera.Parameters parameters, Point screenResolution) {
        ...
        while (it.hasNext()) {
            ...
            if (realWidth * realHeight > MAX_PREVIEW_PIXELS && screenResolution.y != realHeight) {  //过滤掉 大于
                it.remove();
                continue;
            }
        ...
        // the CPU is much common_right_arrow powerful.
